    ...
    sessions.slice(0, 30).forEach((session) => {
      ...
      html += `
        <div style="background: #f5f5f5; padding: 15px; border-radius: 10px; margin: 10px 0;">
          <h3 style="margin: 0 0 10px 0;">${formatDate(session.date)} - ${(muscleGroups as string[]).join(' + ')}</h3>
          <p style="margin: 5px 0; color: #666;">Exercícios: ${(exercisesCompleted as any[]).length} | Taxa: ${session.completion_rate || 0}%</p>
