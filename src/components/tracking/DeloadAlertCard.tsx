...
> = {
  none:        { color: "#4ADE80", bg: "rgba(74,222,128,0.08)",  border: "rgba(74,222,128,0.15)",  icon: Info,         label: "Volume normal" },
...
export const DeloadAlertCard = () => {
  ...
  if (alert.loading) {
    return (
      <div style={{ background: "#141414", borderRadius: 16, padding: "16px", border: "1px solid rgba(255,255,255,0.07)" }}>
        <div style={{ height: 14, width: "55%", background: "rgba(255,255,255,0.07)", borderRadius: 6, animation: "pulse 1.5s infinite" }} />
      </div>
    );
  }
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {alert.severity === "none" ? (
            <p style={{ fontSize: 11, color: "rgba(255,255,255,0.50)" }}>Volume dentro do esperado</p>
          ) : (
        ...
        {alert.severity !== "none" && (
          ...
          >
            <ChevronRight
              size={16} color="rgba(255,255,255,0.30)"
      ...
      {alert.weeklyVolumes.some((v) => v > 0) && (
        <div style={{ marginTop: 14 }}>
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.30)", letterSpacing: "0.12em", marginBottom: 8 }}>
          ...
          <ResponsiveContainer width="100%" height={70}>
            <BarChart data={chartData} barSize={28} margin={{ top: 0, right: 0, left: -32, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: "rgba(255,255,255,0.30)" }} axisLine={false} tickLine={false} />
              <Bar dataKey="volume" radius={[4, 4, 0, 0]}>
                {chartData.map((entry, i) => {
                  ...
                  return (
                    <Cell
                      key={i}
                      fill={isLast && isHigh ? cfg.color : isLast ? "#60A5FA" : "rgba(255,255,255,0.15)"}
                    />
                  );
                })}
              </Bar>
        ...
        {expanded && alert.severity !== "none" && (
          ...
          >
            <div style={{
              ...
              <p style={{ fontSize: 12, color: "rgba(255,255,255,0.70)", lineHeight: 1.6, marginBottom: 12 }}>
                💡 {alert.recommendation}
              </p>
              {alert.fatigueIndex !== null && (
                <p style={{ fontSize: 11, color: "rgba(255,255,255,0.30)", marginBottom: 12 }}>
