...
import {
  ...
  ChevronRight, Clock, Utensils, Info, Check, Sparkles
} from 'lucide-react';
...
const goalConfig = {
  cut: { icon: TrendingDown, label: 'Definição', color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20', gradient: 'from-blue-500/10 to-cyan-500/10' },
  maintain: { icon: Scale, label: 'Manutenção', color: 'text-green-500', bg: 'bg-green-500/10', border: 'border-green-500/20', gradient: 'from-green-500/10 to-emerald-500/10' },
  bulk: { icon: TrendingUp, label: 'Volume', color: 'text-orange-500', bg: 'bg-orange-500/10', border: 'border-orange-500/20', gradient: 'from-orange-500/10 to-red-500/10' },
};
  ...
  return (
    <>
      ...
      <AppModal
        ...
        title={<><Utensils className="w-5 h-5" /> {selectedPlan ? selectedPlan.name : 'Planos Alimentares'}</>}
      >
        <div className="space-y-4">
          <AnimatePresence mode="wait">
            {!selectedPlan ? (
              <motion.div key="plans-list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                {mealPlans && mealPlans.length > 0 ? mealPlans.map((plan) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`w-full p-4 rounded-2xl text-left transition-all border ${isCurrentGoal ? `bg-gradient-to-r ${config.gradient} ${config.border}` : 'bg-card/50 border-border/50 hover:bg-card'}`}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {isCurrentGoal && <span className="px-2 py-0.5 rounded-full bg-primary/20 text-primary text-xs font-medium">Atual</span>}
                          </div>
              ...
              <motion.div key="plan-detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <div className={`p-4 rounded-2xl bg-gradient-to-r ${goalConfig[selectedPlan.goal].gradient} border ${goalConfig[selectedPlan.goal].border}`}>
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div><p className="text-xl font-bold">{selectedPlan.calorieRange.min}-{selectedPlan.calorieRange.max}</p><p className="text-xs text-muted-foreground">kcal/dia</p></div>
                    <div><p className="text-xl font-bold">{selectedPlan.proteinPerKg}g</p><p className="text-xs text-muted-foreground">proteína/kg</p></div>
                    <div><p className="text-xl font-bold">{selectedPlan.days[0].meals.length}</p><p className="text-xs text-muted-foreground">refeições</p></div>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  {selectedPlan.days.map((day, index) => (
                    <button
                      ...
                      className={`flex-1 py-2 px-3 rounded-xl text-sm font-medium transition-all ${selectedDay === index ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground'}`}>
                ...
                <div className="p-3 rounded-xl bg-card/50 border border-border/50">
                  <div className="grid grid-cols-4 gap-2 text-center text-sm">
                    <div className="min-w-0"><p className="font-bold truncate">{Math.round(selectedPlan.days[selectedDay].totals.calories)}</p><p className="text-xs text-muted-foreground">kcal</p></div>
                    <div className="min-w-0"><p className="font-bold text-red-500 truncate">{Math.round(selectedPlan.days[selectedDay].totals.protein)}g</p><p className="text-xs text-muted-foreground">Prot</p></div>
                    <div className="min-w-0"><p className="font-bold text-yellow-500 truncate">{Math.round(selectedPlan.days[selectedDay].totals.carbs)}g</p><p className="text-xs text-muted-foreground">Carbs</p></div>
                    <div className="min-w-0"><p className="font-bold text-blue-500 truncate">{Math.round(selectedPlan.days[selectedDay].totals.fat)}g</p><p className="text-xs text-muted-foreground">Gord</p></div>
                  </div>
                </div>
                    ...
                    return (
                      <div key={index} className="p-3 rounded-xl bg-card/50 border border-border/50">
                        ...
                        <div className="flex flex-wrap gap-1 mb-2">
                          {meal.foods.map((food, i) => <span key={i} className="px-2 py-0.5 rounded-full bg-muted/50 text-xs">{food.name}</span>)}
                        </div>
